...
function migrateTasksStatusForeignKey(db: Database.Database) {
  ...
  db.exec(`
    ...
    INSERT INTO tasks_new (id, user_id, title, description, category_id, status, status_id, progress, due_date, created_at, updated_at, start_date, board_position, location)
    SELECT id, user_id, title, description, category_id, status, status_id, progress, due_date, created_at, updated_at, start_date, board_position, location
    FROM tasks;
    DROP TABLE tasks;
