  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        const rl = checkRateLimit(`login:${identifier}:${normalizedUsername.toLowerCase()}`, { limit: 8, windowMs: 60_000 })
        if (!rl.allowed) return null

        const db = getDb()
        const user = db
          .prepare(
            'SELECT id, username, email, password_hash, display_name, role, is_active, profile_photo FROM users WHERE username = ? COLLATE NOCASE'
          )
