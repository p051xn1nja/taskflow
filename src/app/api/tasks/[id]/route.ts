...
function shiftDate(dateStr: string | null, recurrence: 'daily' | 'weekly' | 'monthly') {
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const db = getDb()
  const task = db.prepare('SELECT * FROM tasks WHERE id = ? AND user_id = ?').get(params.id, session!.user.id) as {
    id: string
  ...
  // Auto-create next instance when recurring task is marked completed
  const becameCompleted = body.status === 'completed' || body.progress === 100
  const recurrence = body.recurrence ?? task.recurrence ?? 'none'
  if (becameCompleted && recurrence !== 'none') {
    ...
    db.prepare(`
      INSERT INTO tasks (id, user_id, title, description, category_id, status, status_id, progress, start_date, due_date, location, recurrence)
    ...
    `).run(
      ...
      shiftDate(task.start_date, recurrence as 'daily' | 'weekly' | 'monthly'),
      shiftDate(task.due_date, recurrence as 'daily' | 'weekly' | 'monthly'),
      task.location || '',
      recurrence
    )
  }
