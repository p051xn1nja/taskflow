...
function TasksPageInner() {
  ...
  const triggerReminderNotification = async () => {
    if (reminderWebhookAvailable === false) {
  ...
  useEffect(() => {
    if (reminderNotificationStatus === 'idle') return
    const timer = setTimeout(() => setReminderNotificationStatus('idle'), 4000)
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="card w-full max-w-md p-5 space-y-3 animate-scale-in">
            ...
            <input
              ...
              placeholder="Task title... (supports: today, tomorrow, next week, next month, next mon)"
              value={quickTitle}
