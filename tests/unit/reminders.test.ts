...
describe('resolveReminderNotificationStatus', () => {
  ...
  it('prioritizes failed status over no_webhook_configured when response is not ok', () => {
    expect(resolveReminderNotificationStatus({
      responseOk: false,
      notificationAvailable: false,
  ...
  it('returns failed when webhook_failed reason conflicts with dispatched=true', () => {
    expect(resolveReminderNotificationStatus({
      responseOk: true,
      notificationAvailable: true,
      notificationReason: 'webhook_failed',
